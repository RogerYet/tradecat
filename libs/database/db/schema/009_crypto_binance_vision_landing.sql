...
CREATE TABLE IF NOT EXISTS crypto.spot_trades (
...
SELECT create_hypertable('crypto.spot_trades', 'time_ts', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);

ALTER TABLE crypto.spot_trades
...
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.spot_trades', INTERVAL '3 days');
...
CREATE TABLE IF NOT EXISTS crypto.futures_um_trades (
    file_id         BIGINT NOT NULL REFERENCES storage.files(file_id),
    symbol          TEXT   NOT NULL,
    id              BIGINT NOT NULL,
    price           NUMERIC(38, 12) NOT NULL,
...
SELECT create_hypertable('crypto.futures_um_trades', 'time_ts', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);

ALTER TABLE crypto.futures_um_trades
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
         timescaledb.compress_orderby = 'time_ts,id');
...
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_um_trades', INTERVAL '3 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;
...
CREATE TABLE IF NOT EXISTS crypto.futures_um_book_ticker (
    file_id             BIGINT NOT NULL REFERENCES storage.files(file_id),
    symbol              TEXT   NOT NULL,
...
SELECT create_hypertable('crypto.futures_um_book_ticker', 'event_time_ts', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);

ALTER TABLE crypto.futures_um_book_ticker
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
...
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_um_book_ticker', INTERVAL '3 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;

-- futures/um/daily/bookDepth/{SYMBOL}/{SYMBOL}-bookDepth-YYYY-MM-DD.csv
-- header：timestamp(datetime),percentage,depth,notional
CREATE TABLE IF NOT EXISTS crypto.futures_um_book_depth (
    file_id         BIGINT NOT NULL REFERENCES storage.files(file_id),
    symbol          TEXT   NOT NULL,
...
SELECT create_hypertable('crypto.futures_um_book_depth', 'timestamp', chunk_time_interval => INTERVAL '7 days', if_not_exists => TRUE);

ALTER TABLE crypto.futures_um_book_depth
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
...
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_um_book_depth', INTERVAL '30 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;
...
CREATE TABLE IF NOT EXISTS crypto.futures_um_metrics (
    file_id         BIGINT NOT NULL REFERENCES storage.files(file_id),
...
SELECT create_hypertable('crypto.futures_um_metrics', 'create_time', chunk_time_interval => INTERVAL '30 days', if_not_exists => TRUE);

ALTER TABLE crypto.futures_um_metrics
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
...
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_um_metrics', INTERVAL '90 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;

-- ==================== crypto.futures_cm（占位：结构与 UM 对称） ====================
-- 注意：当前样本未包含 futures/cm 数据，但 Binance Vision 目录结构存在，字段通常与 UM 相同或高度相似。
-- 先建“占位表”，未来接入 cm 数据时可直接落库。

CREATE TABLE IF NOT EXISTS crypto.futures_cm_trades (LIKE crypto.futures_um_trades INCLUDING ALL);
CREATE TABLE IF NOT EXISTS crypto.futures_cm_book_ticker (LIKE crypto.futures_um_book_ticker INCLUDING ALL);
CREATE TABLE IF NOT EXISTS crypto.futures_cm_book_depth (LIKE crypto.futures_um_book_depth INCLUDING ALL);
CREATE TABLE IF NOT EXISTS crypto.futures_cm_metrics (LIKE crypto.futures_um_metrics INCLUDING ALL);

-- CM 表也按与 UM 相同的策略创建 hypertable + 压缩（当前无数据，先占位不影响）
SELECT create_hypertable('crypto.futures_cm_trades', 'time_ts', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
ALTER TABLE crypto.futures_cm_trades
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
         timescaledb.compress_orderby = 'time_ts,id');
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_cm_trades', INTERVAL '3 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;

SELECT create_hypertable('crypto.futures_cm_book_ticker', 'event_time_ts', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
ALTER TABLE crypto.futures_cm_book_ticker
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
         timescaledb.compress_orderby = 'event_time_ts,update_id');
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_cm_book_ticker', INTERVAL '3 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;

SELECT create_hypertable('crypto.futures_cm_book_depth', 'timestamp', chunk_time_interval => INTERVAL '7 days', if_not_exists => TRUE);
ALTER TABLE crypto.futures_cm_book_depth
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
         timescaledb.compress_orderby = 'timestamp,percentage');
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_cm_book_depth', INTERVAL '30 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;

SELECT create_hypertable('crypto.futures_cm_metrics', 'create_time', chunk_time_interval => INTERVAL '30 days', if_not_exists => TRUE);
ALTER TABLE crypto.futures_cm_metrics
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
         timescaledb.compress_orderby = 'create_time');
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.futures_cm_metrics', INTERVAL '90 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;
...
CREATE TABLE IF NOT EXISTS crypto.option_bvol_index (
    file_id         BIGINT NOT NULL REFERENCES storage.files(file_id),
...
SELECT create_hypertable('crypto.option_bvol_index', 'calc_time_ts', chunk_time_interval => INTERVAL '30 days', if_not_exists => TRUE);

ALTER TABLE crypto.option_bvol_index
    SET (timescaledb.compress = TRUE,
         timescaledb.compress_segmentby = 'symbol',
...
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.option_bvol_index', INTERVAL '180 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;
...
CREATE TABLE IF NOT EXISTS crypto.option_eoh_summary (
    file_id         BIGINT NOT NULL REFERENCES storage.files(file_id),
    date            DATE NOT NULL,
...
SELECT create_hypertable('crypto.option_eoh_summary', 'hour_ts', chunk_time_interval => INTERVAL '30 days', if_not_exists => TRUE);

ALTER TABLE crypto.option_eoh_summary
    SET (timescaledb.compress = TRUE,
...
DO $$
BEGIN
    PERFORM add_compression_policy('crypto.option_eoh_summary', INTERVAL '365 days');
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;
