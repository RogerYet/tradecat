...
- **目标**：
  1. 在 `services/consumption/` 下创建符合项目标准的 api-service
  2. 实现 CoinGlass V4 风格的 API 规范（响应格式、端点命名、参数设计）
  3. 修复 telegram-service `No module named 'psutil'` 错误
- **影响范围**：
  - 新增：`services/consumption/api-service/` 完整服务（由历史 `services-preview/api-service/` 迁移而来）
  - 修改：`services/consumption/telegram-service/requirements.txt`
...
### 改动点 #12
- **文件**：`services/consumption/api-service/src/routers/signal.py`
- **位置**：`get_cooldown_status()` 函数
- **变更类型**：新增（重命名自 signals.py）
- **变更摘要**：端点改为 `GET /api/signal/cooldown`，时间戳改为毫秒

### 改动点 #13
- **文件**：`services/consumption/api-service/src/routers/__init__.py`
- **位置**：模块导出
- **变更类型**：修改
- **变更摘要**：更新路由导出（新增 coins/ohlc/open_interest/funding_rate/futures_metrics/indicator/signal）

### 改动点 #14
- **文件**：`services/consumption/api-service/src/app.py`
- **位置**：路由注册
- **变更类型**：修改
- **变更摘要**：路由前缀改为 `/api/futures`、`/api`（去掉 v1）

### 改动点 #15
- **文件**：`services/consumption/api-service/docs/API_SPEC_COMPARISON.md`
- **位置**：新文件
- **变更类型**：新增
- **变更摘要**：CoinGlass V4 规范对比文档

### 改动点 #16
- **文件**：`services/consumption/api-service/docs/API_EXAMPLES.md`
- **位置**：新文件
- **变更类型**：新增
- **变更摘要**：API 调用示例文档（cURL/Python/JavaScript）

### 改动点 #17-26（基础配置文件）
- **文件**：`services/consumption/api-service/` 下的基础配置文件
...
```bash
# 停止服务
cd services/consumption/api-service && ./scripts/start.sh stop
...
# 或删除整个服务目录
rm -rf services/consumption/api-service

# 回滚 telegram-service 依赖
# 编辑 services/consumption/telegram-service/requirements.txt 删除 psutil 行
```
...
### 修改文件
```
services/consumption/telegram-service/requirements.txt  # 添加 psutil>=5.9.0
services/consumption/telegram-service/requirements.lock.txt  # 更新锁定依赖
```
