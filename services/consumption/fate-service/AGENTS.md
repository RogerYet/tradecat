...
```
tradecat/assets/config/.env          # 生产配置（所有服务共用）
tradecat/assets/config/.env.example  # 配置模板
```
...
from _paths import (
    ...
    CONFIG_DIR,          # assets/config/ 目录
    ENV_FILE,            # assets/config/.env 路径
    BAZI_DB_DIR,         # 数据库目录
    LOGS_DIR,            # 日志目录
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| `未设置 FATE_BOT_TOKEN` | 配置未填写 | 编辑 `assets/config/.env` 添加 |
