...
func getOpenInterestData(symbol string) (*OIData, error) {
	if usingAIPayloadSource() {
		if oi, err := getOpenInterestFromAIPayload(symbol); err == nil {
			return oi, nil
		}
	}
	if usingTradecatSource() {
		return getOpenInterestFromTradecat(symbol)
	}
...
func getOpenInterestFromTradecat(symbol string) (*OIData, error) {
	apiClient := NewAPIClient()
	url := tradecatURL("/open-interest/history")
	...
	if !result.Success {
		return nil, fmt.Errorf("tradecat open interest api failed: %s", result.Msg)
	}
	...
	for _, item := range result.Data {
		sum += parseFloatString(item.Close)
	}
	avg := sum / float64(len(result.Data))
	latest := parseFloatString(result.Data[len(result.Data)-1].Close)
...
func getOpenInterestFromAIPayload(symbol string) (*OIData, error) {
	payload, err := loadAIPayload(symbol)
	if err != nil {
		return nil, err
	}
	if len(payload.Metrics) == 0 {
		return nil, fmt.Errorf("ai payload metrics empty")
	}
	...
	for i := len(payload.Metrics) - 1; i >= 0; i-- {
		value := parseFloatString(payload.Metrics[i].SumOpenInterest)
		if value == 0 {
			continue
		}
		total += value
	...
	if count == 0 {
		return nil, fmt.Errorf("ai payload metrics invalid")
	}
