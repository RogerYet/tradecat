...
function ModelConfigModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const requiresApiKey = selectedModel?.provider !== 'gemini-cli'
              ...
              >
                ...
                {AI_PROVIDER_CONFIG[selectedModel.provider] && (
                  <div className="mt-3 pt-3" style={{ borderTop: '1px solid #2B3139' }}>
                    ...
                    {selectedModel.provider === 'gemini-cli' && (
                      <div className="mt-2 text-xs p-2 rounded" style={{ background: 'rgba(240, 185, 11, 0.1)', color: '#F0B90B' }}>
                        {language === 'zh'
                          ? '本地 CLI 模式：无需 API Key，需确保 gemini 命令可用（可在“自定义 Base URL”里指定命令路径）'
                          : 'Local CLI mode: no API key required. Ensure `gemini` CLI is available (you can set command path in Custom Base URL).'}
                      </div>
                    )}
                  </div>
                )}
            ...
            {selectedModel && (
              <>
                ...
                <div>
                  ...
                  <input
                    type={selectedModel.provider === 'gemini-cli' ? 'text' : 'url'}
                    value={baseUrl}
                    onChange={(e) => setBaseUrl(e.target.value)}
                    placeholder={
                      selectedModel.provider === 'gemini-cli'
                        ? (language === 'zh' ? '可选：CLI 命令路径（默认 gemini）' : 'Optional: CLI command path (default: gemini)')
