"""Futures CM / trades（Raw/基元：逐笔成交）

# ==================== 对齐官方目录语义 ====================
# - rel_path（官方相对路径）：
#   data/futures/cm/daily/trades/{SYMBOL}/{SYMBOL}-trades-YYYY-MM-DD.csv
#
# - CSV header（假设：与 UM 一致；请以 Vision 文件为准）：
#   id,price,qty,quote_qty,time,is_buyer_maker
#
# ==================== 字段设计（强约束：字段完备） ====================
# - id             : 逐笔成交 ID（Binance trade id）
# - price          : 成交价
# - qty            : 成交量（以官方 CSV 为准；实时侧按 ccxt/stream 语义）
...
from src.runtime.vision_paths import relpath_futures_cm_trades_daily
...
from src.writers.raw_futures_cm_trades import RawFuturesCmTradeRow, RawFuturesCmTradesWriter
...
class CmTrade:
    """CM trades 原子事件（对齐官方字段语义）。"""

    symbol: str  # Binance Vision symbol，如 BTCUSD_PERP
    id: int
    price: Decimal
    qty: Decimal
    quote_qty: Decimal
...
def parse_cm_trade_from_ccxt(trade: Dict[str, Any], fallback_symbol: str) -> CmTrade:
    """把 ccxt.pro 的 trade 对象解析成 CM trades（严格字段完备）。
...
def cm_trade_to_csv_row(t: CmTrade) -> Tuple[str, str, str, str, str, str]:
    """对齐官方 CSV 字段顺序与表现形式。"""
...
def _map_vision_symbol_to_ccxt(exchange: Any, vision_symbol: str) -> str:
    """使用 markets_by_id 做映射：BTCUSD_PERP -> BTC/USD:BTC（示例，以实际 markets 为准）。"""
...
async def _watch_trades(
    ...
    ccxt_symbol: str,
    vision_symbol: str,
    out_queue: "asyncio.Queue[CmTrade]",
    ...
    while True:
        try:
            ...
            for tr in trades:
                try:
                    t = parse_cm_trade_from_ccxt(tr, fallback_symbol=vision_symbol)
...
def _flush(
    ...
    db_buffer: List[RawFuturesCmTradeRow],
    *,
    write_csv: bool,
    write_db: bool,
    trades_writer: Optional[RawFuturesCmTradesWriter],
    ...
    if write_db and trades_writer and db_buffer:
        ...
        logger.info("CM trades 入库 inserted=%d attempted=%d", inserted, attempted)
        if meta_writer:
...
def _update_watermark(meta_writer: IngestMetaWriter, rows: List[RawFuturesCmTradeRow], *, exchange: str, dataset: str) -> None:
    if not rows:
        return
...
async def collect_realtime(
    *,
    ...
    """实时采集 CM trades（WS 优先），并按官方目录落盘/入库。"""
    ...
    if not hasattr(ccxtpro, "binancecoinm"):
        raise RuntimeError("ccxtpro 缺少 binancecoinm（无法采集 futures/cm）")
    ...
    db_buffer: List[RawFuturesCmTradeRow] = []
    csv_rows_count = 0
    q: Optional[asyncio.Queue[CmTrade]] = None
    ...
    try:
        ...
        if write_db:
            ...
            trades_writer = RawFuturesCmTradesWriter(conn)
            meta_writer = IngestMetaWriter(conn)
            run_id = meta_writer.start_run(IngestRunSpec(exchange="binance", dataset="futures.cm.trades", mode="realtime"))
        ...
        for s in symbols:
            ...
            logger.info("CM trades WS订阅: %s -> %s", vision_symbol, ccxt_symbol)
        ...
        while True:
            try:
                ...
                for t in batch:
                    rel_path = relpath_futures_cm_trades_daily(t.symbol, t.file_date)
                    ...
                    if write_csv:
                        csv_buffer.setdefault(local_path, []).append(cm_trade_to_csv_row(t))
                        csv_rows_count += 1

                    if write_db and trades_writer:
                        db_buffer.append(
                            RawFuturesCmTradeRow(
                                exchange="binance",
                                symbol=t.symbol,
                                id=t.id,
                                price=t.price,
        ...
        run_status = "failed"
        run_error = str(e)
        logger.error("CM trades 实时采集异常: %s", e, exc_info=True)
        raise
    finally:
...
__all__ = ["collect_realtime", "parse_cm_trade_from_ccxt", "CmTrade"]
