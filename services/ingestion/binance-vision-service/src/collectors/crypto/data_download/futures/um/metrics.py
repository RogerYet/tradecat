...
def _relpath_daily_zip(symbol: str, d: date) -> str:
    sym = symbol.upper()
    return f"data_download/futures/um/daily/metrics/{sym}/{sym}-metrics-{d:%Y-%m-%d}.zip"


def _relpath_monthly_zip(symbol: str, month: str) -> str:
    sym = symbol.upper()
    return f"data_download/futures/um/monthly/metrics/{sym}/{sym}-metrics-{month}.zip"
...
def _ingest_zip(
    ...
    *,
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            """
            INSERT INTO crypto.raw_futures_um_metrics (
              file_id,
              create_time,
              symbol,
              sum_open_interest,
        ...
        cur.execute(
            """
            SELECT COUNT(*)
            FROM crypto.raw_futures_um_metrics
    ...
    if import_writer:
        batch_id = import_writer.start_batch(
            ImportBatchSpec(
                ...
                note="binance_vision futures/um metrics backfill",
                meta={
                    "dataset": dataset,
