...
def _relpath_daily_zip(symbol: str, d: date) -> str:
    sym = symbol.upper()
    return f"data_download/futures/um/daily/trades/{sym}/{sym}-trades-{d:%Y-%m-%d}.zip"


def _relpath_monthly_zip(symbol: str, month: str) -> str:
    sym = symbol.upper()
    return f"data_download/futures/um/monthly/trades/{sym}/{sym}-trades-{month}.zip"
...
def _ingest_zip(
    ...
    *,
    ...
    with conn.cursor() as cur:
        ...
        if force_update:
            chunks_to_recompress = _list_um_trades_compressed_chunks(cur, start_ms=int(start_ms), end_ms=int(end_ms))
    ...
    if import_writer:
        batch_id = import_writer.start_batch(
            ImportBatchSpec(
                ...
                note="binance_vision um trades backfill",
                meta={
                    "dataset": dataset,
