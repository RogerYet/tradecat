"""futures/cm/trades 缺口修复（gap repair）。

逻辑与 futures/um/trades 对称：
- 认领 open gaps（open -> repairing）
- 把 gap 的 [start_time,end_time) 转成 UTC 日期范围
- 调用 Binance Vision 权威回填（daily/monthly 智能边界）
- 成功 close，失败/partial reopen
"""
...
def repair_open_gaps(
    *,
    ...
    run_id = meta_writer.start_run(IngestRunSpec(exchange="binance", dataset="futures.cm.trades", mode="repair"))
    ...
    try:
        ...
        if not claimed:
            logger.info("repair: 没有可认领的 open gaps（exchange=binance dataset=futures.cm.trades）")
